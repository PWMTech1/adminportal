                ...
                <form method="post" action="{{ route('user.update', $user) }}" autocomplete="off"
                    ...
                    <div class="card ">
                        ...
                        <div class="card-body ">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('firstname') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('firstname') ? ' is-invalid' : '' }}"
                                            name="firstname" id="input-name" type="text"
                                            placeholder="{{ __('First Name') }}"
                                            value="{{ old('firstname', $user->firstname) }}" required="true"
                                            aria-required="true" />
                                        @if ($errors->has('firstname'))
                                        <span id="name-error" class="error text-danger"
                                            for="input-name">{{ $errors->first('firstname') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <label class="col-sm-2 col-form-label"
                                    for="input-password-confirmation">{{ __('Clinician Type') }}</label>
                                <div class="col-sm-7">
                                    <div class="form-group">
                                        <select class="form-control" name="cboClinicianType" id="cboClinicianType">
                                            <option></option>
                                            <option value="MD"
                                                {{$user->ClinicianType == "MD" ? "selected='selected'" : ""}}>MD
                                            </option>
                                            <option value="NP"
                                                {{$user->ClinicianType == "NP" ? "selected='selected'" : ""}}>NP
                                            </option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <label class="col-sm-2 col-form-label"
                                    for="input-password-confirmation">{{ __('Percentage') }}</label>
                                <div class="col-sm-7">
                                    <div class="form-group">
                                        <input type="text" id="percentage" name="percentage" class="form-control"
                                            value="{{ old('FFSPercentage', $user->FFSPercentage) }}">
                                    </div>
                                </div>
                            </div>
                        </div>
